...
if __name__ == '__main__':
    ...
    fr_model = load_model_by_repo_id(repo_id=recognition_model_id,
    ...
    for i, row in pairs.iterrows():
        ...
        # read images
        img1 = Image.open(os.path.join(data_root, 'images', row['A']))
        img2 = Image.open(os.path.join(data_root, 'images', row['B']))
    ...
    df.to_csv(os.path.join(data_root, 'result.csv'), index=False)
    print(df)